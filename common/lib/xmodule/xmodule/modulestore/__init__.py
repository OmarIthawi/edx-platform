...
class ModuleStoreWriteBase(ModuleStoreReadBase, ModuleStoreWrite):
    ...
    def _find_course_asset(self, course_key, filename, get_thumbnail=False):
        ...
        all_assets = SortedCollection(course_assets.get(info, []), key=itemgetter('filename'))
        ...
        idx = None
        try:
            _, idx = all_assets.find_with_index(filename)
        except ValueError:
            # Raises this when asset is not found. Catch and return no index.
            pass
    ...
    def _get_all_asset_metadata(self, course_key, start=0, maxresults=-1, sort=None, get_thumbnails=False, **kwargs):
        ...
        all_assets = SortedCollection(course_assets.get(info, []), key=itemgetter(sort_field))
